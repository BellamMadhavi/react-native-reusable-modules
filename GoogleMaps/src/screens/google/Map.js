...
import FullRoundButtonComp from '../config/FullRoundButtonComp';
...
const MapScreen = () => {
  ...
  const openGoogleMaps = () => {
    if (!isSourceValid && !isDestinationValid) {
      setIsSourceValid(true);
      Alert.alert('Invalid Input', 'Please select  source and destination.');
      return;
    }else if (!isSourceValid){
      Alert.alert('Invalid Input', 'Please select  source.');

    }else if (!isDestinationValid){
      Alert.alert('Invalid Input', 'Please select  destination.');
      ...
      if (
        ...
        Alert.alert('Invalid Input','Can`t apply same route options.Try again ');
        return;
      }
